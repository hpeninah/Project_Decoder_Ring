...
function polybius(input, encode = true) {
  ...
  if(encode){
    ...
    const wordsToKeys = lowerCase.map((word) => word.split('').map((letter) => cipher[letter]));
  ...
  if(!encode) {
    ...
    const decodeLetters = separatedWords.map((numberKey) => numberKey.map((letter) => 
      Object.keys(cipher).find((key) => cipher[key] == letter)));
